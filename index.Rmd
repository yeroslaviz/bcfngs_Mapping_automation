...
- Updated `./scripts/bcfngs genomes build` behavior and documentation:
  - `genomes build` now skips `genomes plan` by default.
  - Added `-no-skip-plan` / `--no-skip-plan` to force preflight checks (config + URL checks, lint, dry-run) before build.
- Added optional execution backend parameter for genome builds:
  - `--executor local|slurm` (default: `local`).
  - Added SLURM options for jobs and cluster settings (`--jobs`, partition/account/qos, mail settings, runtime, CPUs, memory, exclusive node).
- Added default SLURM settings for cluster runs:
  - `--partition p.hpcl8`
  ...
  - `--cpus-per-task 20`
  - `--mem-per-cpu 20G`
- Added `snakemake-executor-plugin-slurm` to `env/environment.yml` for Snakemake 8 SLURM execution support.
- Updated Snakemake examples to reflect local default behavior and optional SLURM execution.
