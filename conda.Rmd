...
The first step is to create a conda environment, containing the neccessary tools for the required analysis. In this workflow, we use the `snakemake` package to help running the workflow, as well as some mappers. 
To learn more about the conda environment and its advantages, take a look at the [conda web site](https://conda.io/en/latest/).

Environemnts can be created with the help of a file, e.g. `environment.yaml` using the following command
...
the last part of the environmant file is a list of the needed `dependencies`. Each one of them has its version attached to it. This can be very helpful, when a dataset needes to be reanalyzed to check for problems or conflicts with older/newer results or for debugging purposes.
